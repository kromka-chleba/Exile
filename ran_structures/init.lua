...
ran_structures.init_node_construct = init_node_construct

-- The call of Struct
function ran_structures.call_struct(pos, struct_style, rotation, pr)
	minetest.log("action","[ran_structures] call_struct " .. struct_style.." at "..minetest.pos_to_string(pos))
	if not rotation then
		rotation = "random"
	end
	--[[
	if struct_style == "end_portal_shrine" then
		return ran_structures.generate_end_portal_shrine(pos, rotation, pr)
	end
	]]
end
...
--[[
function ran_structures.generate_end_portal_shrine(pos, rotation, pr)
	local offset = {x=6, y=4, z=6}
	--local size = {x=13, y=8, z=13}
	local newpos = { x = pos.x - offset.x, y = pos.y, z = pos.z - offset.z }

	local path = modpath.."/schematics/ran_structures_end_portal_room_simple.mts"
	ran_structures.place_schematic(newpos, path, rotation or "0", nil, true, nil, shrine_placement_callback, pr)
end
]]
...
function ran_structures.get_structure_data(structure_type)
...
-- ran_structures.get_structure_data.
function ran_structures.register_structure_data(structure_type, structures)
...
dofile(modpath.."/api0.lua")
dofile(modpath.."/loot.lua")
dofile(modpath.."/api.lua")

--structures
dofile(modpath.."/exile_hut.lua")
dofile(modpath.."/exile_hovels.lua")
dofile(modpath.."/exile_water_huts.lua")
...
ran_structures.register_structure("desert_well",{
	place_on = {"group:sand"},
	fill_ratio = 0.01,
	...
	y_offset = -2,
	biomes = { "Desert" },
	filenames = { modpath.."/schematics/ran_structures_desert_well.mts" },
})

ran_structures.register_structure("fossil",{
	...
	biomes = { "Desert" },
	filenames = {
		modpath.."/schematics/ran_structures_fossil_skull_1.mts", -- 4×5×5
		modpath.."/schematics/ran_structures_fossil_skull_2.mts", -- 5×5×5
		modpath.."/schematics/ran_structures_fossil_skull_3.mts", -- 5×5×7
		modpath.."/schematics/ran_structures_fossil_skull_4.mts", -- 7×5×5
		modpath.."/schematics/ran_structures_fossil_spine_1.mts", -- 3×3×13
		modpath.."/schematics/ran_structures_fossil_spine_2.mts", -- 5×4×13
		modpath.."/schematics/ran_structures_fossil_spine_3.mts", -- 7×4×13
		modpath.."/schematics/ran_structures_fossil_spine_4.mts", -- 8×5×13
	},
})

ran_structures.register_structure("boulder",{
	filenames = {
		modpath.."/schematics/ran_structures_boulder_small.mts",
		modpath.."/schematics/ran_structures_boulder_small.mts",
		modpath.."/schematics/ran_structures_boulder_small.mts",
		modpath.."/schematics/ran_structures_boulder.mts",
		-- small boulder 3x as likely
	},
},true) --is spawned as a normal decoration. this is just for /spawnstruct
ran_structures.register_structure("ice_spike_small",{
	filenames = {
		modpath.."/schematics/ran_structures_ice_spike_small.mts"
	},
},true) --is spawned as a normal decoration. this is just for /spawnstruct
ran_structures.register_structure("ice_spike_large",{
	sidelen = 6,
	filenames = {
		modpath.."/schematics/ran_structures_ice_spike_large.mts"
	},
},true) --is spawned as a normal decoration. this is just for /spawnstruct
]]
...
minetest.register_chatcommand("spawnstruct", {
	...
	func = function(name, param)
		...
		local errord = false
		local message = "Structure placed."

		if param == "" then
			message = "Error: No structure name given. Please use “/spawnstruct <type>”."
			errord = true
		else
			for n,d in pairs(ran_structures.registered_structures) do
				if n == param then
					ran_structures.place_structure(pos,d,pr,math.random())
					return true,message
				end
			end
			message = "Error: Unknown structure name. Please use “/spawnstruct <type>”."
			errord = true
		end
...
minetest.register_on_mods_loaded(function()
	local p = ""
	for n,_ in pairs(ran_structures.registered_structures) do
		p = p .. " | "..n
	end
