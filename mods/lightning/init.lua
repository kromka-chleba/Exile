...
local function choose_pos(pos)
   if not pos then
      ...
      pos.x = math.floor(
         pos.x - (lightning.range_h / 2) + rng:next(1, lightning.range_h))
      pos.y = pos.y + (lightning.range_v / 2)
      pos.z = math.floor(
         pos.z - (lightning.range_h / 2) + rng:next(1, lightning.range_h))
   end

   local b, pos2 = minetest.line_of_sight(pos, {x = pos.x,
                                                y = pos.y - lightning.range_v,
                                                z = pos.z}, 1)
...
lightning.strike = function(pos, cosmetic)
   ...
   minetest.add_particlespawner({
         ...
         vertical = true,
         -- to make it appear hitting the node that will get set on fire,
         -- make sure to make the texture lightning bolt hit exactly in
         -- the middle of the texture (e.g. 127/128 on a 256x wide texture)
         texture = "lightning_lightning_" .. rng:next(1,3) .. ".png",
         -- 0.4.15+
         glow = 14,
   })

   minetest.sound_play({ pos = pos, name = "lightning_thunder", gain = 10,
