...
local function on_construct_arch_support(pos)
    if supports[minetest.pos_to_string(pos)] then -- this is our node
        supports[minetest.pos_to_string(pos)] = nil
        -- we're done tracking it now
        return
    end
...
local function check_for_arch(pos, node)
    ...
    for axis, mod in pairs({ ["x"] = 0, ["z"] = 128 }) do
        ...
        if archable(middle.node) == true then
            if archable(left.node) == true and archable(right.node) == true
            then
                fill_arch({ left, right, middle }, axis, mod)
                return
            end
        elseif archable(left.node) == true and archable(right.node) == false
        then
            if do_extend(left, middle, right, mod) then
                return
            end
        elseif archable(left.node) == false and archable(right.node) == true
        then
...
local function drop_arch(ngb, plain_node, fromsupport)
    ...
    repeat
        ...
        if node.name == supportname then
            done = true -- don't go past supports
        end
        count = count + 1
