...
dofile(modpath..'/place_physics.lua')
...
function ncrafting.roast(pos, selfname, name, length, heat, wet_result)
    ...
    --check if wet stop
    if climate.get_rain(pos) or minetest.find_node_near(pos, 1,
                                                        {"group:water"}) then
        if wet_result then
...
function ncrafting.do_bake(pos, elapsed, heat, length, cookname, burnname)
    ...
    elseif temp > fire_temp * 2  or baking < burntime then
        ...
        --Smoke
        minetest.sound_play("tech_fire_small",{pos=pos, max_hear_distance = 10,
                                               loop=false, gain=0.1})
...
function ncrafting.do_soak(pos, name, interval, catchup, detectfunc)
    ...
    if type(detectfunc) == "function" then
        ...
        -- check for water group 1 (freshwater)
        can_soak = can_soak and can_soak.groups and can_soak.groups.water == 1
    end
    if can_soak then
        local meta = minetest.get_meta(pos)
        local soaking = meta:get_int("soaking")
        -- option to calculate soaking via nodetimer elapsed
        --   and nodetimer interval
        soaking = (type(interval) == "number" and type(catchup) == "number")
            and soaking - math.floor(catchup/interval)
            or soaking - 1
    ...
    -- can_soak was not true (no water, unless condition
    --   was specified with other parameters with detectfunc)
    return true
end
