...
local function is_excluded(candidate_name, def)
    if not (candidate_name == candidate_name:gsub("wielded_light","")) then
        return true
    elseif def and def.drop then
        -- digging this gives something else, exclude it
        return true
    else
        return false
    end
...
bundledef = {
    ...
    preserve_metadata = function(pos, oldnode, oldmeta, drops)
        ...
        stack_meta:set_string("description", bundlename(stack_meta,
                                                        bundled_plant,
                                                        treatment))
    end,
}
...
local function add_dye(pos, stack, def)
    ...
    if potdye == 0 or potdye == 31 or
        -- 123 -1 -> 012 = band #0, 345 = band #1
        math.floor((potdye - 1) / 3) ==
