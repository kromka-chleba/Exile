ropes.make_rope_on_timer = function(rope_node_name)
    return function(pos, _elapsed)
        ...
        if currentlength > 1
            and (not minetest.is_protected(newpos, placer_name)
                 or minetest.check_player_privs(placer_name,
                                                "protection_bypass")) then
            if minetest.get_item_group(newnode.name, "air") > 0 then
                minetest.add_node(newpos, {name=currentend.name,
                ...
                minetest.set_node(pos, {name=rope_node_name,
                                        param2=oldnode.param2})
...
ropes.destroy_rope = function(pos, nodes)
    ...
    while not finished do
        local emin, emax = voxel_manip:read_from_map(
            {x=pos.x, y=bottom, z=pos.z}, {x=pos.x, y=top, z=pos.z})
...
ropes.hanging_after_destruct = function(pos, top_node, middle_node, bottom_node)
    ...
    local unlit = wielded_light.get_unlit_node(node)
    if unlit.name == top_node or unlit.name == middle_node
        or unlit.name == bottom_node then
        return
        -- this was done by another ladder or rope node changing
        --  this one, don't react
    end

    pos.y = pos.y + 1 -- one up
    local node_above = minetest.get_node(pos)
    unlit = wielded_light.get_unlit_node(node_above)
    if unlit.name == middle_node then
    ...
    unlit = wielded_light.get_unlit_node(node_below)
    if unlit.name == middle_node then
        ropes.destroy_rope(pos, {middle_node, bottom_node})
    elseif unlit.name == bottom_node then
...
ropes.move_players_down = function(pos, radius)
    local all_objects =
        minetest.get_objects_inside_radius({x=pos.x, y=pos.y+radius, z=pos.z},
            radius)
    for _,obj in pairs(all_objects) do
        if obj:is_player() then
            local obj_pos = obj:get_pos()
            if math.abs(obj_pos.x-pos.x) < 0.5
                and math.abs(obj_pos.z-pos.z) < 0.5 then

                obj:move_to({x=obj_pos.x, y=obj_pos.y-1, z=obj_pos.z}, true)
            end
        end
    end
end
