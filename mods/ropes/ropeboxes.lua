...
local function register_rope_block(multiple, max_multiple, name_prefix,
                                   node_prefix, tint, flammable)
    local node_name = string.format("ropes:%s%irope_block",
                                    node_prefix, multiple)
    local rope_block_def = {
        description = S("@1 Ropebox @2m", name_prefix,
                        ropes.ropeLength*multiple),
        ...
        after_place_node = function(pos, placer)
            ...
            if minetest.is_protected(pos_below, placer_name)
                and not minetest.check_player_privs(placer,
    ...
    if multiple > max_multiple then
        ...
        rope_block_def.drop = string.format("ropes:%s1rope_block %i",
                                            node_prefix, multiple)
    end
...
local rope_def = {
    ...
    after_destruct = function(pos)
        ropes.hanging_after_destruct(pos,
                                     "ropes:rope_top",
                                     "ropes:rope",
                                     "ropes:rope_bottom")
    end,
...
local rope_bottom_def = {
    ...
    on_construct = function( pos )
        minetest.sound_play(
            "ropes_new",{
                pos = pos, gain = (math.random(5,15)/100),
                pitch = (math.random(90,110)/100),
                max_hear_distance = 14, fade = 1,
        })
