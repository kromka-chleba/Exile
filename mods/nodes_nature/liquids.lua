...
local erupt = function(pos, aname, h)
	...
	local height = 0
	--erupt
	while (aname == "air" or aname == "climate:air_temp" or aname == "nodes_nature:lava_flowing") and height < h do
		if ran()<0.8 then
			height = height + 1
			pos.y = pos.y + 1
...
local lava_actions = function(pos, node)
	...
	--do cooling --TODO: this needs to happen instantly and explosively. Currently its a slow unreliable wait.
	...
	--or climate.get_rain(pos) then --these cool things in caves, possibly due to lava being a light source
		lava_particle(pos)
		minetest.set_node(pos, {name = "nodes_nature:volcanic_ash"}) -- PLACEHOLDER! Ash is not technically correct, should be black sand
  ...
  if pos_air then
    ...
    if ran() > 0.63 then
      ...
      --spread instability
      local spos = minetest.find_node_near(pos, 1, {'nodes_nature:lava_source', "nodes_nature:lava_flowing"})
      if spos then
        local pa = 	{x = pos.x, y = pos.y+1, z = pos.z}
        local an = minetest.get_node(pa).name
        if an == 'air' or an == 'climate:air_temp' or an == "nodes_nature:lava_flowing" then
