...
function plant.get_seed_base_props(plant_def)
    ...
    -- spores or seeds (default to seeds)
    local seed_type = (plant_def.lifeform_type == "mushroom" or plant_def.plant_type == "moss") and 
        "spores" or "seeds"
    -- choose between spores texture or seeds texture
    local seed_texture = seed_type == "spores" and "nodes_nature_spores.png" or
        "nodes_nature_seeds.png"
    -- choose spore description if spores, otherwise default to seeds description
    local seed_description = seed_type == "spores" and S("@1 Spores", plant_def.description) or
        S("@1 Seeds", plant_def.description)
    -- plant image to be shown in left corner of seeds
    -- get dead fruiting texture if only_dead_fruit, get fruiting texture if fruiting, or
    -- otherwise use regular plant texture
    local plant_img = plant_def.only_dead_fruit and plant.get_dead_texture_name(plant_def.name) or
        plant_def.fruit and plant.get_fruiting_texture_name(plant_def.name) or 
        plant.get_texture_name(plant_def.name)
    -- generate inventory_image for seed using plant_img crop and seed_texture
    local inventory_seed_image = "([combine:32x32:0,0="..
        plant_img..")^[resize:16x16^"..seed_texture
    local props = {
