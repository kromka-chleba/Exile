...
for i in ipairs(tree_list) do
	...
	minetest.register_node("nodes_nature:"..treename.."_leaves", {
		...
		after_place_node = function(pos, placer, itemstack)
			minetest.set_node(pos, {name = "nodes_nature:"..treename.."_leaves", param2 = 4})
		end,
		after_dig_node = function(pos, oldnode, oldmetadata, digger)
			if oldnode.param2 == 0 then -- mapgen sets param2 to 0
				minetest.set_node(pos, {name = "nodes_nature:tree_mark"})
				local meta = minetest.get_meta(pos)
	...
	if fruitname then
		minetest.register_node("nodes_nature:"..fruitname, {
			...
			after_place_node = function(pos, placer, itemstack)
				minetest.set_node(pos, {name = "nodes_nature:"..fruitname, param2 = p2_fruit})
			end,
			after_dig_node = function(pos, oldnode, oldmetadata, digger)
				if oldnode.param2 == 0 then -- mapgen sets param2 to 0
					minetest.set_node(pos, {name = "nodes_nature:tree_mark"})
