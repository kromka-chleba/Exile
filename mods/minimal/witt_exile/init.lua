-- Zoom WITT
local witt = dofile(minetest.get_modpath('minimal').."/witt_exile/help2.lua")
...
local function old_show(player)
   ...
   nname = ( nname and nname.name )   if not nname then return end
...
local function show(player)
   ...
   local node_definition = minetest.registered_nodes[nname]
   ...
   witt.show(player, meta, nil, node_description, nname,
	     ptd.type, mod_name)
   shown[player] = nname
end
