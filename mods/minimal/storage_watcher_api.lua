...
-- lets you set up proper functionality for when a player is or isn't looking at something, and committing to events accordingly
...
-- "create" is for creating a new watcher table to be used and is assigned to storage_watched
-- shouldn't be used unless you're adding a watcher to the table ; used by add_watcher
...
-- add a watcher
-- adds a player watching storage to the provided pos (or any form of "string" name technically)
function minimal.add_watcher(pos, pname)
  local wt = minimal.get_watchers(pos, true) -- watch_table (2nd boolean will create a new table to add to if there isn't one)
  ...
  -- iterate +1 to get a nil index at the end to add (in case there isn't a random nil index)
  for wi=1,(#wt + 1) do
  ...
  -- due to check for self and check for nil being in the same for loop, it is possible on an off chance for a watcher to
  -- be added twice, Lua is weird!
  -- won't matter beyond getting a genuine count, as remove_watcher will iterate through the entire table to remove all instances
end

-- remove a watcher
-- removes a player watching storage from a specified pos (if it exists and has more than 0 watchers)
