...
-- (may be unclear for translators I guess ?)
local to_s = {
    clay = "Clay",
    wooden = "Wooden",
    freshwater = "Freshwater",
    salt_water = "Salt Water",
    water_pot = "Water Pot", -- TODO if others agree, I would prefer "Pot"
...
-- water is false if empty, true if filled, water by default
-- TODO could be improved with further liquids.
local function get_tiles(mat, container, water)
    ...
    -- if filled, add liquid texture
    if water then
        tiles[1] = tiles[1] .."^tech_pot_water.png"
    end
    return tiles
end
...
for _, container in pairs ({"water_pot", "watering_can"}) do
    for mat, _ in pairs (mats_def) do
        ...
        local def = {
            ...
            -- dapperance
            tiles = get_tiles(mat, container, false),
            drawtype = "nodebox",
        ...
        -- water_pot has additionnal function
        if container == "water_pot" then
...
local function drink(pos, node, clicker, itemstack, pointed_thing, container)
    if drink_water(clicker) then
        minimal.switch_node(pos, container)
...
local function get_base_def(mat, container, liquid)
    local def = {
        ...
        tiles = get_tiles(mat, container, true),
        node_box = nodeboxes[container]
    }
...
for mat, _ in pairs (mats_def) do
    ...
    def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
        drink(pos, node, clicker, itemstack, pointed_thing, def.empty)
    end
...
for _, glass in pairs ({"green", "clear"}) do
    ...
    minetest.register_node(
        "tech:glass_bottle_" .. glass, {
            description = S("@1 Glass Bottle", S(to_s[glass])),
    ...
    for _, liquid in pairs({"salt_water", "freshwater"}) do
        ...
        local def = {
            ...
            description = S("@1 Glass Bottle With @2",
                                S(to_s[glass]), S(to_s[liquid])),
            groups = {dig_immediate = 2},
            tiles  = {tile},
            use_texture_alpha = c_alpha.blend,
            sunlight_propagates = true,
        ...
        elseif liquid == "freshwater" then
            ...
            def.on_rightclick = function(pos, node, clicker, itemstack, pointed_thing)
                if drink_water(clicker) then
                    minimal.switch_node(pos, "tech:glass_bottle_".. glass)
                    minetest.sound_play("nodes_nature_slurp",
                                        {pos = pos, max_hear_distance = 3,
                                         gain = 0.25})
                end
