...
--[[ The inventory formspec is cached for each player like this:
    FS_cache[player_name] = {
        ...
        #TODO check if really needed/used
...
local function new_cache(player)
    ...
    cache.pInv = player:get_inventory()
    ...
    if cache.craft_input == 2 then
        cache.updated = not cache.possible_hint
    end

    -- adds modification and initiat function
...
function cache_func:get_craft_mode()
    ...
    if option then
        ...
        .. "  using default setting " .. input_options[1].label )
        self.craft_input = 1
        return input_options[1]
    end
end
