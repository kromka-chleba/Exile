...
local function make_tool_formspec(player, cache)
    cache = cache or crafting.get_FS_cache(player)
    ...
    fs[#fs + 1] = crafting.make_crafting_formspec(player, nil)

    return tofstring(fs)
end

function crafting.show_station_formspec (player, player_name, cache)
    core.show_formspec(player_name,'exile:crafting',
                                    make_tool_formspec(player, cache))
end
