...
-- #WARNING: it won't be filled until all mods are loaded, don't call it before...
...
local groups_desc = {
    -- example, commented because I felt it was better to register it in tech :
...
* Following fields are created:
    ...
    * `does_match`  is a function taking that gstats and an item as parameter
    (+ its groups as optional param, else we go get them)
    and returning `true` if item matches that grouptag, `false` else.
    * `unit` if present
    ]]
...
function crafting.get_group_stats(grouptag)
    ...
    -- generates full desc of grouptag for recipe (stats.desc) ------

    -- get or generate description for the main group
    stats.name_desc = get_desc(stats.name)

    -- generated tooltip description (name + unit)
    if stats.unit then
...
groups_func.does_match = function(self, item_name, groups)
    ...
    elseif not groups then
        -- get the def and groups of item_name
