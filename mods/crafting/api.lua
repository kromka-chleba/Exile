...
crafting.find_required_items_in_invs = find_required_items_in_invs
...
local function get_replacement(replace, took)
    ...
    -- TODO right now, only ok if 1 size tiem stack in replacement function
...
local function perform_craft(r, name, inv, listname, outlistname, craft_count, sound)
    ...
    -- update r.to_take according to count, if not already done
    -- TODO may e I redo it unneceseraly ?
    r.to_take = r:get_to_take(craft_count)

    local recipe = r.recipe

    -- get list of items required for the recipe (if found)
    local founditems = find_required_items_in_invs(inv, listname,  r.to_take)
...
function crafting.transfer_items(player, pInv, to_list, from_list, item_list)
    if not player then
