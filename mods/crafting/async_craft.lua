...
function default_def.on_timer(pos)
    ...
    local function check_for_craft()
        ...
        local recipes = crafting.get_all(def.craft_type, def.craft_level,
                                         item_hash, unlocked)
        ...
        for _, recipe in pairs(recipes) do
            if recipe.craftable
                and (not best_recipe
                     or #best_recipe.recipe.items < #recipe.recipe.items) then
                best_recipe = recipe
            end
        end
...
function default_def.on_metadata_inventory_put(pos, listname, index,
                                               stack, player)
    local meta = minetest.get_meta(pos)
    meta:set_string("user", player:get_player_name())
    minetest.get_node_timer(pos):start(1.0)
...
function crafting.create_async_station(name, type, level, def, def_active)
    ...
    for key, value in pairs(default_def) do
        ...
        def_active[key] = d_active ~= nil and d_active
            or default_def[key]
    end
