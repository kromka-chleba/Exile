    ...
    if (err ~= nil) then
        if( player ) then
            minetest.chat_send_player(player:get_player_name(),
                                      'Error: Could not save bell data');
        end
        return
    end
    file:write( str );
...
local restore_bell_data = function()
    ...
    if (err ~= nil) then
        print('Error: Could not open bell data savefile '..
              '(ignore this message on first start)');
        return
    end
    local str = file:read();
    file:close();
...
bell.ring_bell = function()
    ...
    local stunde  = tonumber( os.date( '%I'));
    -- in 12h-format (a bell that rings 24x at once would not survive long...)
...
minetest.register_node('artifacts:bell', bell_def)
