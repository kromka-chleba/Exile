...
local default_target = minetest.setting_get_pos("static_spawnpoint")
    or {x=0, y=0, z=0}
...
local function set_target(stack, param)
    ...
    if param.target_name == "" then
        meta:set_string("description",
                        S("Wayfinder to @1",param.target_pos_string))
    else
        meta:set_string("description",
                        S("Wayfinder to @1",param.target_name))
    end
...
local function on_use_function(itemstack, player, pointed_thing)
    ...
    if skip_namechange ~= "" then
        ...
        minetest.show_formspec(
            player:get_player_name(), "wayfinder",
            "size[10,2.5]" ..
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
        if formname == "wayfinder" and fields.name and (fields.ok
                ...
                for j,stack in ipairs(player:get_inventory():get_list("main")) do
                    if string.sub(stack:get_name(), 0, 19) == "artifacts:wayfinder" then
                        player:get_inventory():set_stack(
                            "main", j, get_compass_stack(player, stack))
                    end
                end
            end
        end
...
-- register items
for i = 0, 15 do
    local image = "[combine:16x16:0,0=artifacts_wayfinder_16_base.png:0,"..
        -(i * 16).."=artifacts_wayfinder_16_points.png"
    local groups = {}
    if i > 0 then
