...
local adjust_active_temp = function(pos, temp)
    ...
    --sea water for deep ocean
    --stratification and density set a constant temperature
    ...
    if pos.y <= 0 and water ==0 then
        ...
        --should probably match to heightmap rather than y = 0
        --  (maybe unnecessarily complicated)
...
function climate.heat_transfer(pos, nodename, replace)
    ...
    --  (cools down bc of neighbors).
    --taking them out of temp_pass would allow exploits
    --  (e.g. furnaces built from pots)
    -- it seems good to let air_temp self cool.
    --  Any other temp_pass nodes that ought to be here
...
function climate.air_temp_source(pos, temp_effect, temp_max, chance, timer)
    ...
    local pos_max = {x=pos.x +1, y=pos.y +1, z=pos.z +1}
    local pos_min = {x=pos.x -1, y=pos.y -1, z=pos.z -1}
    local heatable = minetest.find_nodes_in_area(pos_min, pos_max,
                                                 {'air', 'group:heatable'})
...
--  make anything where air movements matter (e.g. fires) produce
--  the above air nodes too
...
--  los can spread through one temp_effect node
--  e.g. to allow stacked fires to work
...
climate.get_point_temp = function(pos, full)
    ...
    if t_effect_max then
        temp = temp + (t_effect_max / 3)
        -- it's partly counted twice, so reduce it
    end
...
climate.get_temp_string = function(v, meta)
    ...
    if meta then
        local tempscalepref = meta:get_string("tempscale")
