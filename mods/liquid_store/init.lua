...
-- TODO curently accepting only one group
...
function liquid_store.register_liquid(source, def)
    ...
    liquid_store.liquids[source] = {
        source = source,
        flowing = def.flowing,
        force_renew = def.force_renew or "false",
...
local function check_input(input)
    ...
    elseif  type(input) == "table" or type(input) == "userdata" then
        input = input.name or type(input.get_name) == "function" and input:get_name() or nil
        if input == "" then
...
local function find_stored(empty, source)
    ...
    -- return container if empty (no source)
    if not source then
        return empty
    end
...
function liquid_store.register_stored_liquid(name,def)
    ...
    -- delete that field if it was present
    ...
    assert(type(container_def) == "table",
        ...
        .. " could not find nodedef for empty variant of '"
        .. name
    ...
    --[[ TODO I think ideal would be to also register liquid, and only use registered container and liquid,
    else send "false" when registering sl_liquid ?]]
    ...
    -- Functions ----------------------------------------------------
    -- TODO do I check containers one if empty ?
...
-- replacement table generation/modification for recipes
--[[ generate of modify replacement tables in recipes for stored liquids
    * `input_sl` is a string, representing item or group to be replaced
    * `new_liquid` is the replacement we want.
    * `replace_t is the recipe's replacement table
...
-- TODO input_sl can be table or other condition
-- if inputs are a table, find the first matching one (no ordered) (stop then)
function liquid_store.replace(item_name, input_sl, new_liquid)
    -- table dealing
    if type(input_sl) == "table" then
        for _, input in pairs (input_sl) do
            local result = liquid_store.replace(item_name, input, new_liquid)
            if result then
                return result
            end
        end
   ...
   if type(input_sl) ~= "string" then
       core.log ("wrong input_sl format in liquid_store.replace")
       return nil
   end
   -- is it a grouptag ? check if item_name matches
   local gstats = crafting.get_group_stats(input_sl)
   if gstats then
       -- if item matchs the grouptag
       if gstats:does_match(item_name) then
           return replace(item_name, new_liquid)
       end
    -- else just test the name
    elseif item_name == input_sl then
        return replace(item_name, new_liquid)
    else
