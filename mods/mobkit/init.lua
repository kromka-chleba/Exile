...
function mobkit.pos_translate2d(pos,yaw,dist)
    -- translate pos dist distance in yaw direction
...
function mobkit.get_terrain_height(pos,steps,dir,liquidflag)
...
function mobkit.isnear3d(p1,p2,thresh)
    if abs(p2.x-p1.x) < thresh
        and abs(p2.z-p1.z) < thresh
        and abs(p2.y-p1.y) < thresh then
...
function mobkit.go_forward_horizontal(self,speed)
    -- sets velocity in yaw direction, y component unaffected
