...
function mobkit.neighbor_shift(neighbor,shift)
   -- int shift: minus is left, plus is right
   return (8+neighbor+shift-1)%8+1
end

function mobkit.is_neighbor_node_reachable(self,neighbor)
   -- todo: take either number or pos
   ...
   if height and abs(height-pos.y) <= self.jump_height then
	 ...
	 for p,node in pairs(nodes) do
	    if snpos.x==p.x and snpos.z==p.z then
		  ...
		  (node.walkable
		   and mobkit.get_node_height(p)>tpos.y+0.001) then return end
	    end
	 end
      end
...
function mobkit.get_next_waypoint(self,tpos)
   ...
   if height and not liq
	 ...
	 if heightr and abs(heightr-height)<0.001 then
	    ...
	    local npos =
	       mobkit.get_node_pos(mobkit.pos_shift(pos,neighbors[neighbor]))
	    local factor = abs(dirn.x) > abs(dirn.z)
	       and abs(npos.x-pos.x) or abs(npos.z-pos.z)
	 ...
	 if height and not liq
      ...
      height, pos2, liq =
	 mobkit.is_neighbor_node_reachable(self,
					   mobkit.neighbor_shift(neighbor,4))
      if height and not liq
	 and not (nogopos and mobkit.isnear2d(pos2,nogopos,0.1)) then
	 update_pos_history(self,pos2)
	 return height,pos2
...
function mobkit.lq_dumbwalk(self,dest,speed_factor)
   ...
   local func=function(self)
      ...
      if mobkit.is_there_yet2d(pos,minetest.yaw_to_dir(
				  self.object:get_yaw()),dest) then
	 -- if mobkit.isnear2d(pos,dest,0.25) then
	 if not self.isonground or abs(dest.y-pos.y) > 0.1 then
	    -- prevent uncontrolled fall when velocity too high
	    --	if abs(dest.y-pos.y) > 0.1 then
	    -- isonground too slow for speeds > 4
...
function mobkit.hq_attack(self,prty,tgtobj)
   local func = function(self)
	 ...
	 if dist > 3 then
	    ...
	    mobkit.lq_turn2pos(self,tpos)
	    local height = tgtobj:is_player() and 0.35
	       or tgtobj:get_luaentity().height*0.6
	    if tpos.y+height>pos.y then
	       mobkit.lq_jumpattack(self,tpos.y+height-pos.y,tgtobj)
	    else
	       mobkit.lq_dumbwalk(self,mobkit.pos_shift(
				     tpos,{x=random()-0.5,z=random()-0.5}))
	    end
	 end
      end
   end
...
function mobkit.is_in_deep(target)
   ...
   if node1 and node2 and node3 and node1.drawtype=='liquid'
      and (node2.drawtype=='liquid' or node3.drawtype=='liquid') then
...
function mobkit.hq_aqua_attack(self,prty,tgtobj,speed)
   ...
   local func = function(self)
	 ...
	 local vel = self.object:get_velocity()
	 if tpos.y>pos.y+0.5 then self.object:set_velocity({x=vel.x,
							    y=vel.y+0.5,
							    z=vel.z})
	 elseif tpos.y<pos.y-0.5 then self.object:set_velocity({x=vel.x,
								y=vel.y-0.5,
								z=vel.z}) end
      end
      if mobkit.is_pos_in_box(mobkit.pos_translate2d(pos,yaw,self.attack.range),
			      tpos,tgtbox) then	--bite
