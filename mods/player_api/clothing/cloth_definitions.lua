...
function player_api.init_cloths(player, pinv)
    ...
    -- #TODO: "cloths" inv is deprecated; remove this after some time, post v4
    do
...
local function migrate_cloths(player, pinv)
    ...
    if cloths_list then
        ...
        -- delete list
        -- #TODO: "cloths" inv is deprecated, remove some time after v4 release
...
function player_api.register_cloth(name, def)
    ...
    -- below function let us equip the cloth using action key (Aux1)
    newdef._on_use_item = function(player, wielded_item, pointed_thing)
        return player_api._on_use_item(wielded_item, player, pointed_thing)
    end
