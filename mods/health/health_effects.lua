...
--[[
    ...
    - HEALTH.remove_new_effect
    (so it can be removed externally - not everything will need this)
    - HEALTH.add_new_effect (so it can be added)


    component effects:
    various minor effects (symptoms) shared by many diseases (e.g. vomiting).
    Called by the health effects internal function
        ...
        minetest.after(
            randel, function()
                ...
                minetest.sound_play("health_vomit",
                                    {pos = pos, gain = 0.5, max_hear_distance = 2})
...
local function stagger(player, meta, repeat_min, repeat_max,
                       delay_min, delay_max, stag)
    local life_num = get_life_num(meta)
...
local function organ_failure(player, repeat_min, repeat_max,
                             delay_min, delay_max, dam_min, dam_max)
    local life_num = get_life_num(player)

    local ranrep = random(repeat_min, repeat_max)
    local name = player:get_player_name()
    append_sound(player,minetest.sound_play("health_heart",
    ...
    if do_timer(meta, "Food Poisoning", 3, 6) == true then
        ...
        if random()<0.1 then
            ...
            default_timer_regress(player, "Food Poisoning", 3, 6, meta,
                                  effects_list, order-1, nil, nil, true)
        end
...
--[[
    ...
    Dust storm born soil fungus got into your lungs.
    Something vaguely like Valley Fever.
...
function HEALTH.drunk(order, player, meta, effects_list, stats)
    ...
    if do_timer(meta, "Drunk", 3, 6) == true then
        ...
        default_timer_regress(player, "Drunk", 3, 6, meta,
                              effects_list, order, nil, {"Hangover",
                                                         max_drunk}, true)
...
function HEALTH.hangover(order, player, meta, effects_list, stats)
    ...
    if do_timer(meta, "Hangover", 4, 8) == true then
        ...
        default_timer_regress(player, "Hangover", 4, 8, meta,
                              effects_list, order, nil, nil, true)
...
if __DEBUG__ then
    ...
    local effects = {
        NS("Food Poisoning"), NS("Dust Fever"), NS("Intestinal Parasites"),
