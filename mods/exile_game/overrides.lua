...
function EXILE.pointed_thing_on_rightclick(itemstack, placer, pointed_thing)
    if pointed_thing.type == "node" and minetest.is_player(placer) then
        local ndef = EXILE.get_nodedef( pointed_thing.under )
...
function EXILE.set_craft_station_on_rightclick(func)
...
local hand_on_place = function(clicker, pointed_thing)
    ...
    if not vector.check(above) or not EXILE.pos_group(above, "air") then
        return false
    end
...
function EXILE.recognize_rapid_placing(itemstack, placer)
...
function minetest.node_dig(pos, node, digger)
    ...
    if minetest.is_player(digger)
        ...
        if full then
            if EXILE.stop_on_inv_full(digger) then
                return false
            end
        end
    end
            ...
            minetest.override_item(
                oName, {
                    preserve_metadata = function(pos, oldnode, oldmeta, drops)
                        ...
                        if imeta then
                            EXILE.metadata.preserve_metadata(imeta,oldmeta)
            ...
            minetest.override_item(
                oName, {
                    after_place_node = function(pos, placer, itemstack,
                        ...
                        EXILE.metadata.after_place_node(imeta,meta)
...
local function get_attached_node_direction(n, group_rating)
    local def = core.registered_nodes[n.name]
    -- if there is a specific functionf or that node, use it
    if type(def._attached_to) == "function" then
        return def._attached_to(n)
    -- else use default behavior from group_rating
    else
        local d = vector.zero()
...
builtin_shared.check_attached_node = function(p, n, group_rating)
    -- get the direction of the node n is attached to
    local d = get_attached_node_direction(n, group_rating)
    -- get the position of that node
    local p2 = vector.add(p, d)
    -- get the name and info of that attached node
