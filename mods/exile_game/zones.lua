...
local zonedef_default = {
    ...
    shape = zs.absolute,
    logarithmic = "false",
    -- Do non-absolute zones fade logarithmically w/distance
...
local function inside_depth(pos, zdef)
    ...
    elseif zdef.shape == zs.cylindrical then
        ...
        depth = ( 1 / ( zdef.narrowest / 2 ) )
            * ( zdef.narrowest / 2 - cdistance )
    end
...
local function hoist_zone(meta, pos) -- Set up new zonedef from meta
    ...
    if not zonelist[zoneid] then -- zone hasn't been seen, add it to the lists
...
local function metaset(meta, def)
    ...
    for key, _ in pairs(zonedef_default) do
        ...
        if vector.check(zonedef_default[key]) -- It's supposed to be a vector!
            and vector.check(val) then -- and it is
    ...
    for nm, _ in pairs(zoneinfo) do
        ...
        if setting then
            if setting == "" then -- erase removed keys
                ...
                meta:set_string("ztrd_"..nm,
                                minetest.serialize(def["ztrd_"..nm]))
            end
        end
    end
...
local function remove_trigger(pos, id)
    ...
    for i = 1, #trigpos do
        if trigpos[count]:equals(pos) then
            ...
            count = count + 1
            -- table.remove shortens the table, use "count" instead of i
        end
    end
...
local function check_triggers_loaded(id, def)
    ...
    for i = 1, #def.triggerpos do
        if def.triggerpos[count] then
            ...
            if node.name ~= "exile_game:zone_trigger" then
...
local function zone_range(def)
    ...
    local formspec =
        "label[0.375,0.5;"..S("Upper corner").."]label[4.35,0.5;"..
        S("( North / Up / East )").."]"..
        ...
        "label[0.375,2.5;"..S("Lower corner").."]label[4,2.5;"..
        S("( South / Down / West )").."]"..
...
function zone_formspec(def) -- actually local, see top of file
    ...
    local formspec = "formspec_version[6]size[10.5,11]"..
        "tabheader[0,0;zt_tabsel;"..S("Range")..","..
        S("Copy")..","..S("Values")..";"..tabsel.."]"
    if tabsel == "1" then
...
minetest.register_node("exile_game:zone_trigger", {
...
if minetest.is_creative_enabled() then
    local zone_ovr = {
        ...
        after_place_node = function(pos, placer, itemstack, pointed_thing, nmeta, imeta)
            ...
            local iinv = EXILE.string2invlists(imeta:get_string("inventory"))
        ...
        preserve_metadata = function(pos, oldnode, oldmeta, drops, imeta)
            ...
            imeta:set_string("inventory", EXILE.invlists2string(list))
    ...
    minetest.override_item('exile_game:zone_trigger', zone_ovr)
end

minetest.register_lbm({
        label = "zonetriggers",
        name = "exile_game:zonetriggers",
        nodenames = "exile_game:zone_trigger",
