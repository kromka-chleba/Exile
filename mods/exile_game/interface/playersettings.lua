...
local theme_fromnum = EXILE.get_gui_theme_list() -- numeric table of theme names
...
local function get_form(playername, meta)
    ...
    local themelist = table.concat(EXILE.get_gui_theme_titles(),",")
...
function EXILE.show_player_settings(playername, meta)
...
EXILE.setting_changed = setting_changed
...
EXILE.register_on_player_setting_change = function(func)
    if type(func) ~= "function" then
        error("EXILE.register_on_player_setting_change: expected function, got type '"..type(func).."'")
    end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if fields.player_settings then
        ...
        EXILE.show_player_settings(name, player:get_meta())
...
local function process_receive_fields(player, formname, fields)
    ...
    if fields.hud16 then
        ...
        EXILE.set_hotbar(player, fields.hud16)
    ...
    if fields.nomusic then
        ...
        --[[ #TODO: put music handling into exile_game where it belongs
