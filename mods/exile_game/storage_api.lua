...
local modname = "exile_game/storage_api.lua"
...
function storage.get_storage_formspec(pos, w, h, meta)
    ...
    local label = EXILE.sanitize_string(meta:get_string('label'))
...
local function can_interact(pos, name, meta)
    ...
    --if EXILE.check_for_lock(pos, name, meta) then return false end
...
function storage.on_receive_fields(pos, formname, fields, sender, width, height)
    ...
    if meta and can_interact(pos,sender, meta) then
        local cleanlabel = EXILE.sanitize_string(label)
        ...
        if #get_watchers(pos) < 1
            ...
            EXILE.sound_play_watcher(pos, nil, false)
        end
    end
end
...
function storage.dump_inventory(pos, meta)
    ...
    if not EXILE.is_group(stor_node.name,"storage") then return end
...
local function to_burnt(pos, meta)
    local stor_node = minetest.get_node(pos)
    if not EXILE.is_group(stor_node.name,"storage") then
    ...
    if not EXILE.is_group(stor_node.burn_to,"storage") then return end
    ...
    EXILE.switch_node(pos, burn_to.name)
end
...
function storage.register_storage(name,def)
    ...
    def.stack_max = def.stack_max or EXILE.stack_max_bulky
    ...
    def.burn_to = def.burn_to
        or def.groups.flammable and "exile_game:burnt_storage_pile"
        or nil
    -- functions
    ...
    -- check exile_game/utility/item.lua for what to expect with inv / target_inv
    ...
    def.storage_inventory_dump_into = def.storage_inventory_dump_into or
        function(stack, count, inv, index, target_inv)
            ...
            if stackdef.groups.backpack then
                ...
                local item_inv = EXILE.get_item_inventory(stack, imeta)
                if item_inv then
...
storage.register_storage("exile_game:burnt_storage_pile",burnt_storage)
