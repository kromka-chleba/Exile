-- exile_game/protection.lua

local S=EXILE.S

local __nail_use_count = 3

function EXILE.protection_is_ownable(pos)
...
function EXILE.protection_key_click(itemstack, clicker, pos, meta)
...
function EXILE.protection_key_use( itemstack, user, pos, meta )
    if not core.is_player(user) then return end
    if not EXILE.protection_is_ownable(pos) then return end -- shouldn't be able to do anything with this
...
function EXILE.protection_nail_use( user, itemstack, pos, meta )
    if not core.is_player(user) then return end
    if not EXILE.protection_is_ownable(pos) then return end -- shouldn't be able to do anything with this
    meta = meta or core.get_meta(pos)
    ...
    if idef.sounds and idef.sounds.nail_down then
        EXILE.sound_play(pos, idef.sounds.nail_down)
    end
    return itemstack
end
...
function EXILE.protection_after_place_node( pos, placer, itemstack,
                                              pointed_thing, nmeta, imeta )
    local pn = placer:get_player_name()
    nmeta = nmeta or core.get_meta(pos)
    nmeta:set_string("owner", pn)
    EXILE.infotext_set_new(pos, nmeta)
    return EXILE.player_in_creative(placer)
end

function EXILE.protection_on_dig(pos,oldnode,digger,meta)
    ...
    if not EXILE.player_in_creative(digger) then
