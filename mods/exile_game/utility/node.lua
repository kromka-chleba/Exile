local S = minetest.get_translator("exile_game")

-- check if a valid meta was given
function EXILE.is_meta(meta)
...
--[[ Swap a node, but run its on_construct, so that
    ...
    -- after_place is optional but if provided, expected to be a table of 3 parameters:
        *  placer (player)
        * itemstack (or itemstack being wielded by player,
         will be grabbed from player if not provided)
         * pointed_thing table
]]
function EXILE.switch_node(pos, node, after_place)
    -- check pos
...
function EXILE.slabs_combine(player, itemstack, pos, combine)
    ...
    EXILE.switch_node(pos, combine)
    itemstack:take_item()
    return itemstack
end

function EXILE.slabs_split_hand(player, pointed_node, pointed_thing,
                                  wielded_item)
    local pos = EXILE.get_usable_position(pointed_thing)
    ...
    if wielded_name == "" then
        ...
        EXILE.switch_node(pos, split_name)
    else
...
function EXILE.node_set_int(pos_or_meta, name, value)
...
function EXILE.node_get_int(pos_or_meta, name)
...
function EXILE.node_set_string(pos_or_meta, name, value)
...
function EXILE.node_get_string(pos_or_meta, name)
    -- meta userdata or pos vector
    local meta = is_meta(pos_or_meta) and pos_or_meta
      or vector.check(pos_or_meta) and core.get_meta(pos_or_meta)
    if not meta then
...
function EXILE.force_place(pos, node)
...
-- Deprecated
function EXILE.shift_pos(pos,change)
    core.log("warning", "EXILE.shift_pos() is deprecated, use pos + vector.new(x,y,z) instead")
    return pos + vector.new(change.x, change.y, change.z)
end
EXILE.pos_shift = EXILE.shift_pos

function EXILE.get_pos_under(pos)
    return vector.new(pos.x, pos.y - 1, pos.z)
end

function EXILE.get_pos_above(pos)
    return vector.new(pos.x, pos.y + 1, pos.z)
end

-- get_nodedef: gets node definition and returns it with the get_node as well
function EXILE.get_nodedef(pos)
    local node = core.get_node(pos)
    local ndef = core.registered_nodes[node.name]
...
function EXILE.pos_group(pos, group_name)
...
function EXILE.safe_landing_spot(pos)
    if not vector.check(pos) then return false end
    local floor = vector.new( pos.x, pos.y-1, pos.z )
    local def_top = EXILE.get_nodedef(pos + vector.new(0,1,0))
    local def_bot = EXILE.get_nodedef(pos)
    local def_flr = EXILE.get_nodedef(floor)
    ...
    for i = 0, 20 do
        ...
        def_flr = EXILE.get_nodedef(floor)
...
function EXILE.on_rightclick(itemstack, user, pointed_thing, aboveorunder)
    ...
    local nodedef, node = EXILE.get_nodedef(pos)
...
function EXILE.dig_up(pos, node, digger)
...
function EXILE.get_usable_position(pt, ...)
