...
function EXILE.player_in_creative(plyr)
    ...
    if (minetest.is_player(plyr)) then
        if (minetest.check_player_privs(plyr,"creative")
            or creative_mode_cache == true) then
            return true
        end
    end
    return false
...
do
    ...
    if minetest.settings:get_bool("unlimited_player_transfer_distance",
                                  true) then
    ...
    minetest.register_chatcommand("ping", ping_cmd)
end
