...
--[[
function ran_structures.is_disabled(structname)
	return table.indexof(disabled_structures,structname) ~= -1
end
]]

function ran_structures.fill_chests(p1,p2,loot,pr)
	for it,lt in pairs(loot) do
		local nodes = minetest.find_nodes_in_area(p1, p2, it)
		for _,p in pairs(nodes) do
			local lootitems = ran_st_loot.get_multi_loot(lt, pr)
			ran_structures.init_node_construct(p)
			local meta = minetest.get_meta(p)
			local inv = meta:get_inventory()
			ran_st_loot.fill_inventory(inv, "main", lootitems, pr)
		end
	end
end
...
local function generate_loot(pos, def, pr)
	...
	if def.loot then ran_structures.fill_chests(p1,p2,def.loot,pr) end
end
...
function ran_structures.find_lowest_y(pp)
	local y = 31000
...
function ran_structures.find_highest_y(pp)
	local y = -31000
	for _,p in pairs(pp) do
		if p.y > y then y = p.y end
	end
...
local function find_ground(pos,nn,gn)
	...
	for _,v in pairs(nn) do
		...
		until not n or n == "ignore" or n == gn
...
local function get_foundation_nodes(ground_p1,ground_p2,pos,sidelen,node_stone)
	local replace = {
		"air","group:water","group:boulder","group:tree",
		"group:woody_plant","group:flora",
		"group:sediment"}
...
local function foundation(ground_p1,ground_p2,pos,sidelen)
	--game specific, ideally needs generalizing
	local node_stone = "nodes_nature:limestone"
	local node_filler = "nodes_nature:clay"
	local node_top = "nodes_nature:grassland_soil" or minetest.get_node(ground_p1).name
	local node_dust = nil
...
function ran_structures.place_structure(pos, def, pr, blockseed)
	...
	if def.filenames then
		...
		if file then
			...
			ran_structures.place_schematic(pp, file, "random", nil, true, "place_center_x,place_center_z",function(p)
			...
			if log_enabled then
				minetest.log("action","[ran_structures] "..def.name.." placed at "..minetest.pos_to_string(pp))
			end
			return true
		end
	...
	if log_enabled then
		minetest.log("warning","[ran_structures] placing "..def.name.." failed at "..minetest.pos_to_string(pos))
	end
end
...
function ran_structures.register_structure(name,def,nospawn) --nospawn means it will be placed by another (non-nospawn) structure that contains it's structblock i.e. it will not be placed by mapgen directly
		...
		if def.place_on then
			minetest.register_on_mods_loaded(function() --make sure all previous decorations and biomes have been registered
				def.deco = minetest.register_decoration({
					name = "ran_structures:deco_"..name,
	...
	ran_structures.registered_structures[name] = def
end

--lbm for secondary structures (structblock included in base structure)
minetest.register_lbm({
	name = "ran_structures:struct_lbm",
	run_at_every_load = true,
	nodenames = {"group:structblock_lbm"},
	action = function(pos, node)
		minetest.remove_node(pos)
		local name = node.name:gsub("ran_structures:structblock_","")
		local def = ran_structures.registered_structures[name]
		if not def then return end
		ran_structures.place_structure(pos)
	end
})
