    ...
    for z = min.z, max.z do
        for x = min.x, max.x do
            ...
            for y = min.y, top_y do
                local underground_ok = not underground or
                    (height and minp.y + y < height - underground)
...
function Geomorph:write_sphere(shape, rot, loc)
    ...
    local proportions = vector.divide(vector.subtract(max,
                                                      vector.subtract(min, 1)),
                                      radius * 2)
    ...
    for z = min.z, max.z do
        ...
        for x = min.x, max.x do
            ...
            for y = min.y, top_y do
                ...
                local underground_ok = not underground
                    or (height and minp.y + y < height - underground)
...
function Geomorph:write_puzzle(shape, rot, loc)
    ...
    if self.gpr:next(1, math.max(1, chance)) == 1 then
        ...
        local l = vector.add(shape.location,
                             vector.floor(vector.divide(shape.size, 2)))
